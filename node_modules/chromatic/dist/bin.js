...
var chunkQVYZ34K7_js = require('./chunk-QVYZ34K7.js');
...
`,U=/^\s*([\w.-]+)\s*=\s*(.*)?\s*$/,Y=/\\n/g,G=/\n|\r|\r\n/;function E(e,t){let c=!!(t&&t.debug),i={};return e.toString().split(G).forEach(function(r,s){let a=r.match(U);if(a!=null){let l=a[1],o=a[2]||"",n=o.length-1,g=o[0]==='"'&&o[n]==='"';o[0]==="'"&&o[n]==="'"||g?(o=o.substring(1,n),g&&(o=o.replace(Y,M))):o=o.trim(),i[l]=o;}else c&&k(`did not match key and value when parsing line ${s+1}: ${r}`);}),i}function Q(e){let t=P.resolve(process.cwd(),".env"),c="utf8",i=!1;e&&(e.path!=null&&(t=e.path),e.encoding!=null&&(c=e.encoding),e.debug!=null&&(i=!0));try{let r=E(W.readFileSync(t,{encoding:c}),{debug:i});return Object.keys(r).forEach(function(s){Object.prototype.hasOwnProperty.call(process.env,s)?i&&k(`"${s}" is already defined in \`process.env\` and will not be overwritten`):process.env[s]=r[s];}),{parsed:r}}catch(r){return {error:r}}}m.exports.config=Q;m.exports.parse=E;});var _={};chunkORGADLRC_js.d(_,{main:()=>V});async function V(e){let{code:t}=await chunkQVYZ34K7_js.j({argv:e});process.exit(t);}var D=chunkORGADLRC_js.b(()=>{chunkQVYZ34K7_js.m();});var B={};chunkORGADLRC_js.d(B,{main:()=>X});async function X(e){let{flags:t,input:c}=(0, F.default)(`
    ...
    `,{argv:e,description:"Trace utility for TurboSnap",flags:{statsFile:{type:"string",alias:"s",default:z||"storybook-static/preview-stats.json"},storybookBaseDir:{type:"string",alias:"b",default:H||"."},storybookConfigDir:{type:"string",alias:"c",default:J||".storybook"},untraced:{type:"string",alias:"u",isMultiple:!0},mode:{type:"string",alias:"m"}}}),i={log:console,options:{storybookBaseDir:t.storybookBaseDir,storybookConfigDir:t.storybookConfigDir,untraced:t.untraced,traceChanged:t.mode||!0}},r=await chunkQVYZ34K7_js.g(t.statsFile),s=c.map(l=>l.replace(/^\.\//,"")),a=s.find(l=>chunkQVYZ34K7_js.b(l));if(a)throw new Error(`Unable to trace package manifest file (${a}) as that would require diffing file contents.`);await chunkQVYZ34K7_js.e(i,r,t.statsFile,s);}var F,H,J,z,T=chunkORGADLRC_js.b(()=>{F=chunkORGADLRC_js.e(chunkQVYZ34K7_js.a());chunkQVYZ34K7_js.f();chunkQVYZ34K7_js.h();chunkQVYZ34K7_js.c();({STORYBOOK_BASE_DIR:H,STORYBOOK_CONFIG_DIR:J,WEBPACK_STATS_FILE:z}=process.env);});var C={};chunkORGADLRC_js.d(C,{main:()=>ee});async function ee([e="./storybook-static/preview-stats.json"]){try{let c=(await chunkQVYZ34K7_js.g(e)).modules.filter(A).map(({id:r,name:s,modules:a,reasons:l})=>{let o=Z(l.filter(A).map(n=>n.moduleName)).filter(n=>n!==s).map(n=>({moduleName:n}));return {id:r,name:s,modules:a&&a.map(n=>({name:n.name})),reasons:o}}).filter(Boolean),i=e.replace(".json",".trimmed.json");await(0,I.outputFile)(i,JSON.stringify({modules:c},null,2).replace(/{\n {10}/g,"{ ").replace(/\n {8}}/g," }")),console.log(`Wrote ${i}`);}catch(t){console.error(t);}}var I,Z,A,R=chunkORGADLRC_js.b(()=>{I=chunkORGADLRC_js.e(chunkQVYZ34K7_js.d());chunkQVYZ34K7_js.h();Z=e=>Array.from(new Set(e)),A=({name:e,moduleName:t=e})=>t&&!t.startsWith("(webpack)")&&!t.match(/(node_modules|webpack\/runtime)\//);});O().config();var j={main:()=>(D(),chunkORGADLRC_js.f(_)).main(process.argv.slice(2)),trace:()=>(T(),chunkORGADLRC_js.f(B)).main(process.argv.slice(3)),"trim-stats-file":()=>(R(),chunkORGADLRC_js.f(C)).main(process.argv.slice(3))};(j[process.argv[2]]||j.main)();
